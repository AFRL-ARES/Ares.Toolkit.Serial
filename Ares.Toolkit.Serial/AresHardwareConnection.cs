...
{
  public AresHardwareConnection(string portName, 
    int baudRate = 9600, 
    Parity parity = Parity.None, 
    int dataBits = 8, 
    StopBits stopBits = StopBits.One, 
    SerialConnectionOptions? options = null) : this(new SerialPortConnectionInfo(baudRate, parity, dataBits, stopBits), portName, options)
  {
  }
  ...
  protected override Task AcquireStreamLock(CancellationToken token = default)
  {
    if(SharedPort is null)
      throw new InvalidOperationException("Cannot acquire stream lock without an open shared serial port.");
  ...
  {
    if(SharedPort is null)
      throw new InvalidOperationException("Cannot release stream lock without an open shared serial port.");
  ...
  protected override void Open(string portName)
  {
    ...
    SharedPort = SharedSerialPort.Acquire(this, portName, ConnectionInfo);
